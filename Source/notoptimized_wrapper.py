...
class Wrapper(torch.nn.Module):
    ...
    def getwheeldata(self, wheelind: int, wheel_list: WheelData):
        ...
        with profiler.record_function("Wheelorient"):
            wheelorient2d = self.wheeldir(wheelorient)

        return hmap_w, glob_w, wheelpos, wheelorient2d
    
    def newgetwheeldata(self, wheelind: int, wheel_list: WheelData, wheelpos: Tensor):

        #pos, wheelpos, wheelorient, linvel, angvel = wheel_list[0], wheel_list[1], wheel_list[2], wheel_list[3], wheel_list[4]
        #pos, wheelpos, wheelorient, linvel, angvel = pos.to(self.device), wheelpos.to(self.device), wheelorient.to(self.device), linvel.to(self.device), angvel.to(self.device)
        
        with profiler.record_function("To device"):
            pos = wheel_list[0].to(self.device)
    ...
    def forward(self, w_0: WheelData, w_1: WheelData, w_2: WheelData, w_3: WheelData, verbose: bool = True):
        ...
        with profiler.record_function("Pre"):
            ...
            #     # pos, wheelpos, wheelorient, linvel, angvel
            #     print(w[0].shape, w[1].shape, w[2].shape, w[3].shape, w[4].shape)
            ...
            for wheelind in range(self.batchsize):
                ...
                #hmap_w = self.newgetwheeldata(wheelind, wheels_info[wheelind], self.wheelpos[wheelind])
