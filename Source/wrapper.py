...
class Wrapper(torch.nn.Module):
    ...
    def __init__(self, model, namemodel):
        ...
        self.window_x = self.wheel_radius
        self.window_y = self.wheel_semiwidth*1.3
    ...
    def to_wheelframe(self, wheelframe_step, x_step, wheelpos_step, glob_step, rot90, batch):

        wheelframe_ort = self.get_ort2d(wheelframe_step, rot90)
    ...
    def get_hmap_wrapper(self, pcloud, wheelpos, sink):
    ...
    def hmap2pcloud(self, hmap, wheelpos):
    ...
    def getwheeldata(self, wheelind: int, wheel_list: WheelData):
        ...
        hmap_w = self.get_hmap_wrapper(pos, wheelpos, sinkage)
        # DEBUGGGG
        ...
        return hmap_w, glob_w, wheelpos

    def forward(self, w_0: WheelData, w_1: WheelData, w_2: WheelData, w_3: WheelData, verbose: bool = True):

        self.verbose = verbose

        wheeltype = torch.zeros(4, dtype=torch.int64, device=self.device)

        wheels_info = [w_0,w_1,w_2,w_3]

        num_parts = [w_i[0].shape[0] for w_i in wheels_info]

        hmap = torch.zeros((4, self.n_channels, self.sizegrid, self.sizegrid), device=self.device)
        glob = torch.zeros((4,6), device=self.device)
        wheelpos = torch.zeros((4,3), device=self.device)
        ...
        for wheelind in range(self.batchsize):
            ...
            hmap_w, glob_w, wheelpos_w = self.getwheeldata(wheelind, wheels_info[wheelind])
            hmap[wheelind] = hmap_w
            glob[wheelind] = glob_w
            wheelpos[wheelind] = wheelpos_w

        wheeltype = wheeltype.view(self.batchsize,1)

        outsoil = self.model(hmap, glob)

        soil0, soil1, soil2, soil3 = self.hmap2pcloud(outsoil, wheelpos)

        #print(outsoil.shape, soil2.shape)
